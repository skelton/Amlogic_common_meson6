...
//#define CONFIG_FOCALTECH_TOUCHSCREEN_CODE_UPG_DOIT
...
static int ft5x0x_get_event(struct ts_event *event)
{
	...
	if (focaltechPdata2->ypol)
		...
		if (event->x > focaltechPdata2->screen_max_x - offset) event->x = focaltechPdata2->screen_max_x - offset;
		if (event->y < 20) event->y = 20;
		if (event->y > focaltechPdata2->screen_max_y - offset) event->y = focaltechPdata2->screen_max_y - offset;
...
//	ts =  container_of(handler, struct ft5x0x_ts_data, early_suspend);
	...
	//it will cause a bug,for sometime it will inactive for short power down and up
	//if(focaltechPdata2->power) {
		// u8 data;
		// ft5x0x_write_reg(0xa5, 0x03);
		// printk("set reg[0xa5] = 0x03\n");
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct input_dev *input_dev;
	int err = 0;
	unsigned char uc_reg_value,file_ver;
...
#ifdef CONFIG_FOCALTECH_TOUCHSCREEN_CODE_UPG_DOIT
	msleep(50);
	//get some register information
	uc_reg_value = ft5x0x_read_fw_ver();
	printk("[FST] Firmware version in TP is = 0x%x\n", uc_reg_value);
	file_ver = fts_ctpm_get_upg_ver();
	printk("[FST] Firmware version in File is = 0x%x\n", file_ver);
	if(uc_reg_value < file_ver){
		fts_ctpm_fw_upgrade_with_i_file();
		mdelay(200);
		uc_reg_value = ft5x0x_read_fw_ver();
		printk("[FST] Firmware version in TP after update is = 0x%x\n", uc_reg_value);
	}
	else{
		printk("[FST] No need update!\n");
	}
#endif
