...
#else
    input_report_key(ts->input_dev, BTN_TOUCH,  1);  //yj
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
...
#else
    input_report_key(ts->input_dev, BTN_TOUCH,  0);  //yj
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    {
        //GTP_GPIO_AS_INT(GTP_INT_PORT);	
        ts->client->irq = FT_IRQ;
	gpio_direction_input(GPIO_FT_IRQ);
#if GTP_INT_TRIGGER==1
	gpio_enable_edge_int(gpio_to_idx(GPIO_FT_IRQ), 0, FT_IRQ - INT_GPIO_0); //INT_RISING
#elif GTP_INT_TRIGGER==0
    ...
    //GTP_GPIO_AS_INPUT(GTP_RST_PORT);
    //set_gpio_mode(GPIOC_bank_bit0_15(3), GPIOC_bit_bit0_15(3), GPIO_INPUT_MODE);
    gpio_direction_input(PAD_GPIOC_3);
    gtp_reset_guitar(20);
