...
static const char *s3c_ts_name = "goodix_gt811";
...
static struct goodix_i2c_rmi_platform_data *goodixPdata;
#ifdef DEBUG
int sum = 0;
int access_count = 0;
int int_count = 0;
...
//	printk("%s\n",__func__);
...
#ifdef CONFIG_GOODIX_GT811_SELECT_MID7303CM
	uint8_t config_info[] = {
	0x06,0xA2,
	
	0x12,0x10,0x0E,0x0C,0x0A,0x08,0x06,0x04,0x02,0x00,0xE7,0x77,0xD7,0x77,0xC7,0x77,
	0xB7,0x77,0xA7,0x77,0x97,0x77,0x87,0x77,0x77,0x77,0x67,0x77,0x57,0x77,0x47,0x77,
	0x37,0x77,0x27,0x77,0x17,0x77,0x07,0x77,0xF7,0x77,0x17,0x03,0xC8,0xC8,0xC8,0x32,
	0x32,0x32,0x0F,0x0F,0x0A,0x45,0x35,0x1D,0x03,0x00,0x05,0xE0,0x01,0x20,0x03,0x00,
	0x00,0x3B,0x35,0x37,0x31,0x00,0x00,0x26,0x14,0x02,0x05,0x00,0x00,0x00,0x00,0x00,
	0x14,0x10,0x4E,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x0D,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01
	};
#else //following is used for MID7306CE
	uint8_t config_info[] = {
	0x06,0xA2,
	
	0x12,0x10,0x0E,0x0C,0x0A,0x08,0x06,0x04,0x02,0x00,0xF3,0x33,0xE3,0x33,0xD3,0x33,0xC3,
	0x33,0xB3,0x33,0xA3,0x33,0x93,0x33,0x83,0x33,0x73,0x33,0x63,0x33,0x53,0x33,0x43,0x33,
	0x33,0x33,0x23,0x33,0x13,0x33,0x03,0x33,0x1F,0x03,0x10,0x10,0x10,0x26,0x26,0x26,0x0F,
	0x0F,0x0A,0x40,0x30,0x25,0x03,0x00,0x05,0xE0,0x01,0x20,0x03,0x00,0x00,0x3B,0x33,0x37,
	0x30,0x00,0x00,0x23,0x19,0x02,0x06,0x80,0x00,0x00,0x00,0x00,0x14,0x10,0x4E,0x02,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x01
    
	};
#endif
	config_info[62] = goodixPdata->abs_y_max >> 8;
    config_info[61] = goodixPdata->abs_y_max & 0xff;
    config_info[64] = goodixPdata->abs_x_max >> 8;
    config_info[63] = goodixPdata->abs_x_max & 0xff;
	...
	ts->abs_x_max = goodixPdata->abs_x_max;//(config_info[62]<<8) + config_info[61];
	ts->abs_y_max = goodixPdata->abs_y_max;//(config_info[64]<<8) + config_info[63];
	ts->max_touch_num = config_info[60];
	ts->int_trigger_type = ((config_info[57]>>3)&0x01);
	//width = TOUCH_MAX_WIDTH;
	//height = TOUCH_MAX_HEIGHT;
	dev_info(&ts->client->dev, "GT811 init info:X_MAX=%d,Y_MAX=%d,TRIG_MODE=%s\n TOUCH_MAX_WIDTH===%d,TOUCH_MAX_HEIGHT===%d\n",
	ts->abs_x_max, ts->abs_y_max, ts->int_trigger_type?"RISING EDGE":"FALLING EDGE",goodixPdata->abs_y_max,goodixPdata->abs_x_max); 
	return 0;
}
...
COORDINATE_POLL:
    ...
    if (goodixPdata->swap_xy)
    	swap(input_x, input_y);
    if (goodixPdata->xpol)
			input_x = goodixPdata->abs_x_max - input_x;
    if (goodixPdata->ypol)
			input_y = goodixPdata->abs_y_max - input_y;
			//report point
			//printk("report input_x = %d,input_y=%d \n",input_x,input_y);			
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  char test_data = 1;
	...
	goodixPdata = client->dev.platform_data;

	printk("gt811 enter probe!!\n");
	...
	if (goodixPdata->irq_init)
			goodixPdata->irq_init();
	...
	{
		dev_info(&client->dev,"i2c addr ======== 0x%x \n",client->addr);
...
/////////////////////////////// UPDATE STEP 1 START/////////////////////////////////////////////////////////////////
	...
	input_set_abs_params(ts->input_dev, ABS_X, 0,  goodixPdata->abs_x_max, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_Y, 0, goodixPdata->abs_y_max, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 255, 0, 0);
	
#ifdef GOODIX_MULTI_TOUCH
	input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0,  goodixPdata->abs_x_max, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, goodixPdata->abs_y_max, 0, 0);	
#endif	
  ...
  ret  = request_irq(client->irq, goodix_ts_irq_handler,IRQF_DISABLED,"GTP_INT_IRQ", ts);
	//printk("gt811 client->irq = %d", client->irq);
	if (ret < 0) {
		dev_err(&client->dev, "gt811_probe: request irq failed\n");
...
#endif
	if (ts->power) {	/* 必须在取消work后再执行，避免因GPIO导致坐标处理代码死循猿*/
		ret = ts->power(ts, 0);
		if (ret < 0)
...
static  int gt811_reset( struct goodix_ts_data *ts )
{
	...
	for(retry=0;retry <= 2; retry++)
	{
		...
		ret =i2c_write_bytes(ts->client, &test_data, 1);	//Test I2C connection.
		if (ret >= 0)
			break;
...
static u8  gt811_update_proc( u8 *nvram, u16 start_addr , u16 length, struct goodix_ts_data *ts )
{
	...
	CLEAR_CBUS_REG_MASK(PAD_PULL_UP_REG0, (1<<16));
...
begin_upgrade:
 ...
 //   goto exit_downloader;
    ...
    ret = gpio_request(ts->client->irq, "TS_INT");	//Request IO
    if (ret < 0) 
    {
        dev_info(&ts->client->dev,"Failed to request GPIO:%d, ERRNO:%d\n",(int)ts->client->irq,ret);
        err = -1;
        goto exit_downloader;
    }
