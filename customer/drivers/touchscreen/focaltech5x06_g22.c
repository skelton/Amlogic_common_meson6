...
static int ft5x0x_get_event(struct ts_event *event)
{
	...
	for (i=0; i<ret; i++) {
	  ...
	  /*event->y = (s16)(buf[3+i*6] & 0x0F)<<8 | (s16)buf[4+i*6];   //EDT 2012/11/19 by ETML0700E8DHA ->800*1280 TO 1280*800 正旋90度
		event->x =1280-((s16)(buf[5+i*6] & 0x0F)<<8 | (s16)buf[6+i*6]);   //EDT 2012/11/19 by ETML0700E8DHA ->800*1280 TO 1280*800 正旋90度
		
		*/
    event->z = 200;
    event->w = 1;
    //printk("prevent->x1= %d prevent->y1= %d\n",event->x,event->y);
