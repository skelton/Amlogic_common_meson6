...
s32 goodix_init_panel(struct goodix_ts_data *ts)
{
   ...
   // u8 *config = (u8 *)ts->pdata->data; config  ts-pdata->data_len (sizeof(config_info)/sizeof(config_info[0])) 
    ...
    {
        dev_info(&(ts->client->dev),"init panel success liujq\n");
        i2c_end_cmd(ts);
        msleep(500);
    }
    return success;
...
static s32 goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
        DEBUG_MSG(" liujq Reques EIRQ %d successed\n", ts->client->irq);
    }
