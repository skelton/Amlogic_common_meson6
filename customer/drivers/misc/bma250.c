...
#define BMA_REGS_BACKUP_RESTORE
#ifdef BMA_REGS_BACKUP_RESTORE
#include <linux/fs.h>
...
#ifdef BMA_REGS_BACKUP_RESTORE
static int bma_regs_br_flag = 0;
static struct i2c_client *bma_i2c_client = 0;

static int bma_acc_i2c_read(unsigned char chip, unsigned int adr, int addr_len, unsigned char *buf, int len)
{
	unsigned short slave_addr_backup = bma_i2c_client->addr;
	int ret;

	bma_i2c_client->addr = chip;
	ret = bma250_smbus_read_byte_block(bma_i2c_client, adr, buf, len);

	if (ret < 0)
	{
		printk(KERN_ERR"read error: slave_addr=%02x, reg_addr=%02x, err=%d\n", chip, adr, ret);
	}
	else
	{
		printk(KERN_INFO"read: slave_addr=%02x, reg_addr=%02x, buf[0]=0x%02x\n", chip, adr, buf[0]);
	}
	bma_i2c_client->addr = slave_addr_backup;
	return ret;
}
/*----------------------------------------------------------------------------*/
static int bma_acc_i2c_write(unsigned char chip, unsigned int adr, int addr_len ,const unsigned char *buf, int len)
{
	unsigned short slave_addr_backup = bma_i2c_client->addr;
	int ret;
	unsigned char data;
	bma_i2c_client->addr = chip;
	data = buf[0];
	ret = bma250_smbus_write_byte(bma_i2c_client, adr, &data);

	if (ret < 0)
	{
		printk(KERN_INFO"read error: slave_addr=%02x, reg_addr=%02x, err=%d\n", chip, adr, ret);
	}
	else
	{
		printk(KERN_INFO"read: slave_addr=%02x, reg_addr=%02x, buf[0]=0x%02x\n", chip, adr, buf[0]);
...
/*----------------------------------------------------------------------------*/
static int bma_fs_read(const unsigned char * name, unsigned char * buf, int size)
{
	...
	if (err != size) 
	{
		printk(KERN_ERR"%s: Can't read the data from file:%s\n", __func__,name);
		err = -EIO;
	}
...
/*----------------------------------------------------------------------------*/
static int bma_fs_write(const unsigned char * name, unsigned char * buf, int size)
{
	struct file *p_fs = NULL;
	int err = 0;
	mm_segment_t old_fs;
	...
	if (IS_ERR(p_fs)) 
	{
		printk(KERN_ERR"%s: Can't open file %s error=%d\n", __func__, name,(int)p_fs);
		set_fs(old_fs);
		err = PTR_ERR(p_fs);
		return err;
	}
	...
	if (err != size) 
	{
		printk(KERN_ERR"%s: Can't write the data to file:%s\n", __func__,name);
		err = -EIO;
	}
	...
	printk(KERN_INFO "%s write ok\n",name);
	return err ;
}
struct bma_callback bma_cb = {
		.i2c_read	=	bma_acc_i2c_read,
		.i2c_write	=	bma_acc_i2c_write,
		.fs_read	=	bma_fs_read,
		.fs_write	=	bma_fs_write,
		.msleep 	=	msleep,
		.sensor_i2c_id	=	0x18,
};
#endif //BMA_REGS_BACKUP_RESTORE
...
static ssize_t bma250_debug_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bma250_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#ifdef BMA_REGS_BACKUP_RESTORE
			if (bma_regs_br_flag == 1)
			{
				int err = backup_or_restore_i2c(&bma_cb);
				if (err == 0)
				{
					printk(KERN_INFO"bma regs backup or restore successfully");
				}
				else
				{
					printk(KERN_INFO"bma regs backup or restore failed");
				}
				bma_regs_br_flag = 0;
			}
#endif // #ifdef BMA_REGS_BACKUP_RESTORE			
...
static DEVICE_ATTR(debug, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_debug_show, bma250_debug_store);
...
#ifdef BMA_REGS_BACKUP_RESTORE
	bma_i2c_client = client;
	if ((err = detect_bma_sensor(&bma_cb)) == 0)
	{
		bma_regs_br_flag = 1;
	}
	else if(err == -2)
