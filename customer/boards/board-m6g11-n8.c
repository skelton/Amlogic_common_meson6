/*
 * arch/arm/mach-meson6/board-m6g11-n8.c
 *
...
{
	#include "../drivers/touchscreen/firmware/GDM_Release_V1.00A.mgar"
};
...
#ifdef CONFIG_TC103
static struct platform_device t3_i2c_device={
    .name="twx",
};
#endif
...
static struct aml_camera_i2c_fig1_s gc0308_custom_init_script[] = {
    {0x14, 0x11},
    //{0x57, 0x80},// r ratio 0x7e
    //{0x58, 0x80},// g ratio 0x80
    //{0x59, 0x80},//b ratio 0x82
    {0x93, 0x44},
    {0xff, 0xff},
};
...
static struct aml_camera_i2c_fig_s gt2005_custom_init_script[] = {
    //{0x0268, 0x8f},//0x93},//8f r2cr
    //{0x0269, 0xa3},//94},//a3 g2cr
    //{0x026A, 0xb4},//0xB0},//b4 b2cb
    //{0x0441, 0x4b},//4b
    {0xffff, 0xff},
};
...
#ifdef CONFIG_TC103
    {
      I2C_BOARD_INFO("twx_tc103",  0x6e >> 1),
    },
    {
      I2C_BOARD_INFO("twx_tc103_bl",  0x6d >> 1),
    },
#endif
};
...
static struct i2c_board_info __initdata aml_i2c_bus_info_b_bma250[] = {
	{
 ...
 *	  H		1					BMA222
 ***********************************************************************************/
int extern_check_gsensor_type(void)
{
	int val;
	...
	printk("Gsensor Type:%s %d\n", val ? "BMA222" : "MMA8452", val);
	return val;
}
EXPORT_SYMBOL(extern_check_gsensor_type);
...
static int __init aml_i2c_init(void)
{
    ...
    if (extern_check_gsensor_type()) {
		#if defined(CONFIG_BOSCH_BMA250) || defined(CONFIG_BOSCH_BMA250_MEMSIC)
		i2c_register_board_info(1, aml_i2c_bus_info_b_bma250,
			ARRAY_SIZE(aml_i2c_bus_info_b_bma250));
		#endif
	} else {
		#ifdef CONFIG_SENSORS_MMA8452
 ...
 ***********************************************************************************/
int extern_check_wifi_type(void)
{
	int val;
	...
	printk("Wifi Type:%s %d\n", val ? "BCM40183(GB9663)" : "BCM40181(GB9662)", val);
	return val;
}
EXPORT_SYMBOL(extern_check_wifi_type);
...
static void wifi_gpio_init(void)
{
	...
	if (extern_check_wifi_type()) { //GB9663
		...
		aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<11)); //GPIOX-15
		gpio_set_status(PAD_GPIOX_15,gpio_status_in); 
		aml_clr_reg32_mask(P_PAD_PULL_UP_REG4,0xf|1<<8|1<<9|1<<12|1<<15);		
	}
	aml_clr_reg32_mask(P_PAD_PULL_UP_REG2,1<<8);	// GPIOC_8
}
...
void wifi_dev_init(void)
{
	...
	extern_wifi_set_power(0);
	udelay(400);
	extern_wifi_set_power(1);
}
 ...
 **********************************************************************/
/**
			 E8HD-3D        E8HD & E8HD-T3
*BT_RST		 GPIOC_9		GPIOD_2			
*BT_EN		 GPIOC_7        GPIOD_5	
*/
...
//tmp fix by Elvis Yu
static void m6g11_n8_set_vccx2(int power_on)
{
    if (power_on) {
 ...
 **********************************************************************/
static struct platform_device  *platform_devs_with_bt[] = {
#if defined(CONFIG_I2C_AML) || defined(CONFIG_I2C_HW_AML)
    &aml_i2c_device_a,
    &aml_i2c_device_b,
    &aml_i2c_device_ao,
...
static struct platform_device  *platform_devs_without_bt[] = {
#if defined(CONFIG_I2C_AML) || defined(CONFIG_I2C_HW_AML)
    &aml_i2c_device_a,
    &aml_i2c_device_b,
    &aml_i2c_device_ao,
...
static void mmc_lp_early_suspend(struct early_suspend *h)
{
...
static void mmc_lp_late_resume(struct early_suspend *h)
{
    // Enable MMC_LP_CTRL.
    printk("MMC_LP_CTRL1 before=%#x\n", aml_read_reg32(P_MMC_LP_CTRL1));
...
#endif
static struct early_suspend mmc_es;
static __init void mmc_lp_suspend_init(void)
{
#ifdef CONFIG_HAS_EARLYSUSPEND
    mmc_es.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
    mmc_es.suspend = mmc_lp_early_suspend;
    mmc_es.resume = mmc_lp_late_resume;
    register_early_suspend(&mmc_es);
#endif
}

static void set_chromo_saturation(void)
{
    // chromo saturation 0x1d45:0x1300000, 0x1d46:0x130
    aml_write_reg32(P_VPP_VADJ2_MA_MB, 0x1300000);
    aml_write_reg32(P_VPP_VADJ2_MC_MD, 0x130);
}
...
static __init void meson_init_machine(void)
{
	...
	if (extern_check_wifi_type()){
    	platform_add_devices(platform_devs_with_bt, ARRAY_SIZE(platform_devs_with_bt));
	}else {
    	platform_add_devices(platform_devs_without_bt, ARRAY_SIZE(platform_devs_without_bt));
	}
#if defined(CONFIG_I2C_AML) || defined(CONFIG_I2C_HW_AML)
    aml_i2c_init();
#endif
...
MACHINE_START(MESON6_G11_N8, "Amlogic Meson 6 g11 n8 board")
