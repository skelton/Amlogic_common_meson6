...
#include <linux/aml_gpiokey.h>

static pinmux_item_t remote_pinmux_item[] = {
    {
        .reg = AO,
        .clrmask  = (1<<0),
    },
...
static pinmux_set_t remote_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &remote_pinmux_item[0],
};



static inline int gpio_key_init_func(void)
{
	printk("home key gpio init \n");
	//clean remote pinmux first
	pinmux_clr(&remote_pinmux_set);
			
	gpio_set_status(PAD_GPIOAO_7, gpio_status_in);
	return 0;
}

static struct gpio_key_cfg gpio_keys[] = {
	{PAD_GPIOAO_7, KEY_HOME, 1},
};

struct gpio_key_platform_data gpio_key_data = {
	.key_cfgs = &gpio_keys[0],
	.key_num=ARRAY_SIZE(gpio_keys),
	.init_func = gpio_key_init_func,
	};
...
static aml_plat_cam_data_t video_gc0307_data = {
    .name="video-gc0307",
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC2015)|defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC2035)
    .video_nr=1,   //    1
#else
    .video_nr=0,   //    1
#endif
...
static struct aml_camera_i2c_fig1_s custom_gt2015_script[] = {
    //{0x17,0x14}, //0x14   normal , 0x15 h--flip, =0x16  v--flip, =0x17  180?¨¨
    {0xff,0xff},
};
...
#endif
#ifdef CONFIG_GSl1680_CAPACITIVE_TOUCHSCREEN_G12_ANDA_M709A5
    {
