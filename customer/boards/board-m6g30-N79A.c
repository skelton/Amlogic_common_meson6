...
static inline int get_board_type(void)
{
	return gpio_in_get(PAD_GPIOAO_6);
}
...
static struct regulator_init_data mx_vcck_init_data = {
...
static struct meson_cs_pdata_t mx_vcck_pdata = {
	.meson_cs_init_data = &mx_vcck_init_data,
	.voltage_step_table = {
...
static struct meson_opp mx_vcck_opp_table[] = {
	{
		.freq   = 1500000,
		.min_uV = 1358000,
		.max_uV = 1358000,
...
static struct regulator_init_data mxs_vcck_init_data = {
    .constraints = { /* VCCK default 1.2V */
        .name = "vcck",
...
static struct meson_cs_pdata_t mxs_vcck_pdata = {
    .meson_cs_init_data = &mxs_vcck_init_data,
    .voltage_step_table = {
    #ifdef CONFIG_AML1212
...
static struct meson_opp mxs_vcck_opp_table[] = {
...
{
	if (get_board_type())
		return meson_vcck_cur_max_freq(vcck, mx_vcck_opp_table, ARRAY_SIZE(mx_vcck_opp_table));
	else
		return meson_vcck_cur_max_freq(vcck, mxs_vcck_opp_table, ARRAY_SIZE(mxs_vcck_opp_table));
}

static int vcck_scale(unsigned int frequency)
{
	if (get_board_type())
		return meson_vcck_scale(vcck, mx_vcck_opp_table, ARRAY_SIZE(mx_vcck_opp_table),
					frequency);
	else
		return meson_vcck_scale(vcck, mxs_vcck_opp_table, ARRAY_SIZE(mxs_vcck_opp_table),
					frequency);
}
...
//	{KEY_MENU,		"menu", CHAN_4, 9, 40},
	{KEY_HOMEPAGE,	"vol-", CHAN_4, 150, 40},
	{KEY_BACK,		"vol+", CHAN_4, 275, 40},
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_N79A
#include <linux/ft5x06_n79a.h>
...
{
	gpio_set_status(get_board_type()?GPIO_FT_WAKEUP:PAD_GPIOC_3, gpio_status_out);
	gpio_out(get_board_type()?GPIO_FT_WAKEUP:PAD_GPIOC_3, on);
}
...
static struct aml_camera_i2c_fig1_s gc0308_custom_init_script[] = {
	{0xfe,0x00},
	{0x14,0x11},  //13-h--flip-12; 10-v--flip-11
	{0xff,0xff},
};
...
static struct aml_camera_i2c_fig1_s gc2035_custom_init_script[] = {
	{0xfe,0x00},
	{0x17,0x14},//0x14   normal , 0x15 h--flip, =0x16  v--flip, =0x17
	{0xff,0xff},
};
...
//set status
	...
	gpio_set_status(get_board_type()?PAD_GPIOC_8:PAD_GPIOX_11,gpio_status_out);
...
{
	gpio_set_status(get_board_type()?PAD_GPIOC_8:PAD_GPIOX_11,gpio_status_out);//set wifi_en gpio mode out
	if(is_on){
		gpio_out(get_board_type()?PAD_GPIOC_8:PAD_GPIOX_11,0);
		printk("WIFI  Enable! \n");
	}
	else{
		gpio_out(get_board_type()?PAD_GPIOC_8:PAD_GPIOX_11,1);
...
static __init void meson_init_machine(void)
{
	...
	//mmc_lp_suspend_init();
	if (!get_board_type()) {
		meson_cs_dcdc_regulator_device.dev.platform_data = &mxs_vcck_pdata;
		vcck_pwm_init();
	} else {
		meson_cs_dcdc_regulator_device.dev.platform_data = &mx_vcck_pdata;
	}
	setup_usb_devices();
...
MACHINE_START(MESON6_G12, "Amlogic Meson6 Yuke N79A customer platform")
