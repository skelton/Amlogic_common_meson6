/*
 * customer/boards/board-m6g05-n97a.c
 *
...
static int g05_pmu_call_back(void *para)
{
...
static struct axp_supply_init_data axp_sply_init_data = {
  ...
  //.led_control     =  led_control,
    ...
    .pmu_call_back   =  g05_pmu_call_back,
    .board_battery   = &g05_battery,
};
...
static struct adc_key adc_kp_key[] = {
    {KEY_MENU,                 "menu", CHAN_4, 146, 40},
    {KEY_VOLUMEDOWN,    "vol-", CHAN_4, 275, 40},
    {KEY_VOLUMEUP,          "vol+", CHAN_4, 10, 40},
    {KEY_BACK,                "back", CHAN_4, 392, 40},
    {KEY_HOME,                "home", CHAN_4, 510, 40},
};
...
extern void act8xxx_set_outn_poweroff(int outn);  
/*
...
//	val = gpio_in_get(PAD_GPIOA_20);
	logd("%s: get from gpio is %d.\n", __FUNCTION__, val);
//	printk("pmu waibu is online func is %d\n",!val);
	return !val;
}
...
static void power_off(void)
{
    ...
    //VCCx2 power down
 //   set_vccx2(0);
        SET_CBUS_REG_MASK(PAD_PULL_UP_REG0, (1<<26));	//enable internal pullup
        gpio_set_status(PAD_GPIOA_26,gpio_status_out);
        gpio_out(PAD_GPIOA_26,1);
    ...
    //power off out4 for new board
    act8xxx_set_outn_poweroff(4);
}
...
static int bat_int_resistor = 140;
#define MAX_RES_V 175
#define MIN_RES_V 105
static int charging_dir = -2;
static int pre_current0, pre_current1;
static int pre_vol0, pre_vol1;
static int cal_res_flag = 0;
static int cur_vol;
static int cur_currents;

static void cal_bat_resistor(int cur_bat_cur)
{
    int val;
    int cur_bat_vol;
    int cur_diff, vol_diff;
    int temp_v;
retry:
	...
	cur_bat_vol =  val * (4 * 2500000 / 1023);	//单节 2*,双节 4*
	
	if (cur_bat_cur > pre_current1)
	    cur_diff = cur_bat_cur - pre_current1;
	else
	    cur_diff = pre_current1 - cur_bat_cur;
	    
	if (cur_bat_vol >  pre_vol1)
	    vol_diff = cur_bat_vol - pre_vol1;
	else 
	    vol_diff = pre_vol1 - cur_bat_vol;
	    
	if (cur_diff == 0 || vol_diff == 0)
	    return ;

	temp_v = (vol_diff * 1000) / cur_diff;
	
	if (temp_v > MAX_RES_V || temp_v < MIN_RES_V)  {
	    printk("calculated value is %d, the value is wrong, so discard it\n", temp_v);
	    return ;
	}
    
    bat_int_resistor = temp_v;
	pre_current0 = pre_current1 = cur_bat_cur; //update bat current info
	pre_vol0 = pre_vol1 = cur_bat_vol; //update bat voltage info
	
	printk("calculated the new battery internal resistor is: %d \n", bat_int_resistor);
	return ;
}

static void updat_pre_bat_info(int cur_bat_cur)
{
    int val;
    pre_current1 = pre_current0;
    pre_current0 = cur_bat_cur;
retry:
	val = get_bat_adc_value();
	if(val == -1){
	    goto retry;
	...
	pre_vol1 = pre_vol0;
	pre_vol0 =  val * (4 * 2500000 / 1023);	//单节 2*,双节 4*
}
    
static void check_resistor_cal(int now_current, int cur_charging_dir)
{
    if (cur_charging_dir != charging_dir) {
        charging_dir = cur_charging_dir;
        if (charging_dir != -2) { //the init value 
            if (cal_res_flag) {
                cal_res_flag = 0;
                return ;
            } else
    ...
    if (cal_res_flag) {
        cal_res_flag = 0;
        printk("charging state changed!\n");
        cal_bat_resistor(now_current);     //only at the moment of charging state swiching, we have the chance to cal the battery internal resistor.
    } else
    updat_pre_bat_info(now_current); //update battery info for bat internal resistor calculation
}
...
//	set_gpio_val(GPIOA_bank_bit0_27(22), GPIOA_bit_bit0_27(22), 0);
	...
	vdiff = Vbatn - Vgnd;
	val = (vdiff*1000)/(20*20);
	
	check_resistor_cal(val, is_ac_online()?1:-1); //check if there is a chance to recaculate the battery internal resistor.
...
retry:
	...
	val =  val * (4 * 2500000 / 1023);	//单节 2*,双节 4*
	
	Icur = _measure_current();
	Vbat = val - Icur*bat_int_resistor/1000;  // 0.102
	//printk("v:%d vbat:%d\n", val, Vbat);
        cur_vol = Vbat;
	#endif
	return Vbat;
...
{
	return cur_vol;//_measure_voltage();
}

static inline int measure_current(void)
{
	return cur_currents;//_measure_current();
}

static int bat_level_table[57]={
0,
0,
1,
2,
...
static int new_bat_value_table[57]={
0,  //0    
7000000,//0  
7083840,//1
7127670,//2
...
{
    int vbat = _measure_voltage() - 10000; //-10mv
...
#ifdef CONFIG_GOODIX_TOUCH_GT819
...
static u8 gt82x_config[] = {
    ...
    0x0C,0x1B,0x0D,0x1C,0x0E,0x1D,0x00,0x0A,
    0x01,0x0B,0x02,0x0C,0x03,0x0D,0x04,0x0E,
    0x05,0x0F,0x06,0x10,0x07,0x11,0x08,0x12,
    0x09,0x13,0x09,0x03,0x88,0x10,0x10,0x2D,
    0x00,0x00,0x08,0x00,0x00,0x0E,0x50,0x30,
    0x3D,0x03,0x00,0x05,0x00,0x03,0x00,0x04,
    0x00,0x40,0x49,0x3C,0x44,0x05,0x00,0x05,
    0x19,0x25,0x14,0x10,0x02,0xDA,0x01,0x00,
...
#include "board-m6g05-n10a-touch.fw"
};
...
*WIFI_EN		-->GPIOC_8
*BT/WLAN_RST		-->GPIOC_9   //the pin 40183 only used for BT
*BT_EN			-->GPIOC_7
*WIFI_WAKE		-->GPIOX_11
...
static void bt_device_init(void)
{
	...
	/* UART_TX */
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<27));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<21));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<25));
	aml_set_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<9));
	/* UART_RX */
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<30));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<26));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<20));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<24));
	aml_set_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<8));
	/* UART_CTS_N */
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<29));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<25));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<19));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<23)); 
	aml_set_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<7));
	/* UART_RTS_N */
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<28));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_8,(1<<24));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<18));
	aml_clr_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<22)); 
	aml_set_reg32_mask(P_PERIPHS_PIN_MUX_4,(1<<6));
	/* PCM_CLK */
...
#if defined(CONFIG_AM_WIFI_SD_MMC)
    [1] = {
        .name           = "sdio_card",
        .work_mode      = CARD_HW_MODE,
...
static __init void meson_init_machine(void)
{
    ...
    gpio_out(PAD_GPIOA_26,0);
#ifdef CONFIG_ANDROID_TIMED_GPIO
	gpio_out(PAD_GPIOC_14, 0);  //vibrate GPIO, set it to high,let it not vibrate
#endif
}
...
MACHINE_START(MESON6_G05_N97A, "Amlogic Meson6 g05-n97a customer platform")
