...
static int w28_pmu_call_back(void *para)
{
...
static struct axp_supply_init_data axp_sply_init_data = {
  ...
  //.led_control     =  led_control,
    ...
    .pmu_call_back   =  w28_pmu_call_back,
    .board_battery   = &w28_battery,
};
...
#if defined(CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN) || defined(CONFIG_FT5X_UPDATE_TOUCHSCREEN)
...
{
      printk( "amlogic wifi driver: wifi_gpio_init. \n");
//set pinmux 
...
#define HP_UNPLUG   0
    ...
    if(get_display_mode() != 0) {   //if !panel, return HP_PLUG        return HP_PLUG;
	//printk("111rt5631_hp_detect  level=%d,get_display_mode()=%d\n ",level,get_display_mode());            
	return HP_PLUG;
    }
    ...
    //printk("222rt5631_hp_detect  level=%d,get_display_mode()=%d\n ",level,get_display_mode());            
    //if (level == 0) {
    if (level == 1) {   //honghaoyu 耳机与喇叭检测修�?        val = HP_PLUG;
    }
