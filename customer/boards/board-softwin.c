...
static int softwin_pmu_call_back(void *para)
{
...
static struct axp_supply_init_data axp_sply_init_data = {
  ...
  //.led_control     =  led_control,
    ...
    .pmu_call_back   =  softwin_pmu_call_back,
    .board_battery   = &softwin_battery,
};
...
static struct adc_key adc_kp_key[] = {
    ...
    {KEY_SPACE,         "select", 	CHAN_4, 9, 	40},
    {KEY_ENTER,         "start", 	CHAN_4, 392, 	40},
...
#if defined(CONFIG_ADC_JS)
#include <linux/input.h>
#include <linux/adc_keypad.h>

static struct adc_key adc_js_key[] = {
    {KEY_UP, 		"up", 		CHAN_5,	0,	200},
    {KEY_DOWN, 		"down",		CHAN_5, 1023, 	200},
    {KEY_LEFT, 		"left",		CHAN_6, 0, 	200},
    {KEY_RIGHT,		"right", 	CHAN_6, 1023, 	200},
};

static struct adc_kp_platform_data adc_js_pdata = {
    .key = &adc_js_key[0],
    .key_num = ARRAY_SIZE(adc_js_key),
};

static struct platform_device adc_js_device = {
    .name = "mx-adcjs",
    .id = 0,
    .num_resources = 0,
    .resource = NULL,
    .dev = {
        .platform_data = &adc_js_pdata,
    }
};
#endif
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0308_BACK
static int gc0308_back_have_inited = 0;
static pinmux_item_t gc0308_back_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t gc0308_back_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &gc0308_back_pins[0]
};

static int gc0308_back_init(void)
{
    pinmux_set(&gc0308_back_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    ...
    printk("gc0308_back_init OK!!!!\n");
    return 0;
}

static int gc0308_back_v4l2_init(void)
{
    gc0308_back_have_inited=1;
    gc0308_back_init();
}
static int gc0308_back_v4l2_uninit(void)
{
    gc0308_back_have_inited=0;
    printk( "amlogic camera driver: gc0308_back_v4l2_uninit. \n");
...
static void gc0308_back_v4l2_early_suspend(void)
{

}

static void gc0308_back_v4l2_late_resume(void)
{
#if 0
    pinmux_set(&gc0308_pinmux_set);
...
static struct aml_camera_i2c_fig1_s gc0308_back_custom_init_script[] = {
    {0x14,0x10},  //0x10  11
    {0xff,0xff},
};

static aml_plat_cam_data_t video_gc0308_back_data = {
    .name="video-gc0308_back",
    .video_nr=0,
    .device_init= gc0308_back_v4l2_init,
    .device_uninit=gc0308_back_v4l2_uninit,
    .early_suspend = gc0308_back_v4l2_early_suspend,
    .late_resume = gc0308_back_v4l2_late_resume,
    .custom_init_script = gc0308_back_custom_init_script,
};
#endif
...
static struct i2c_board_info __initdata aml_i2c_bus_info_a[] = {
    ...
    {
        /*gc0308 i2c address is 0x42/0x43*/
        I2C_BOARD_INFO("gc0308_back_i2c",  0x44 >> 1),
        .platform_data = (void *)&video_gc0308_back_data,
    },
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GT2005
    {
...
#endif
#if defined(CONFIG_GOODIX_GT82X_CAPACITIVE_TOUCHSCREEN) || defined(CONFIG_GOODIX_GT818X_CAPACITIVE_TOUCHSCREEN)
    {
...
static void power_off(void)
{
	...
	axp_read(&axp->dev, POWER20_STATUS, &val);
        printk("--------- val = 0x%x ----------\n" , val);
	//set charge LED control by charge function
	axp_clr_bits(&axp->dev, POWER20_OFF_CTL, 0x38);
	if(val & ((1<<7) | (1<<5))) {
		printk("------------- charging reboot ----------------\n");
...
MACHINE_START(MESON6_SOFTWIN, "Amlogic Meson6 softwin customer platform")
