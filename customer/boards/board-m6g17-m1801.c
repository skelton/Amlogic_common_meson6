...
static pinmux_item_t ov3660_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t ov3660_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &ov3660_pins[0]
};
static void ov3660_init(void)
{
    pinmux_set(&ov3660_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    printk( "amlogic camera driver: ov3660_v4l2_init. \n");
    gpio_out(PAD_GPIOZ_0, 0);
    gpio_out(PAD_GPIOE_10, 1);
    msleep(20);
    gpio_out(PAD_GPIOZ_0, 1);
...
static void ov3660_v4l2_disable(void)
{
}
...
static aml_plat_cam_data_t video_ov3660_data = {
    .name="video-ov3660",
    .video_nr=0,   //    1
    .device_init= ov3660_v4l2_init,
    .device_uninit=ov3660_v4l2_uninit,
    .early_suspend = ov3660_v4l2_early_suspend,
    .late_resume = ov3660_v4l2_late_resume,
    .device_disable=ov3660_v4l2_disable,
    .custom_init_script = NULL,
   // .flash_support = 1,
    //.flash_ctrl = &ov3660_flashlight_data,
};
#endif /* CONFIG_VIDEO_AMLOGIC_CAPTURE_ov3660 */
...
#endif
#ifdef CONFIG_GSl2682_CAPACITIVE_TOUCHSCREEN_G17_L8
    {
...
//set status
    ...
    gpio_set_status(PAD_GPIOAO_9,gpio_status_out);
...
static void pmu_set_enable(int is_on)
{
	DBG_LINE_INFO();
    gpio_set_status(PAD_GPIOAO_9,gpio_status_out);
    ...
    if(is_on){
        ...
        axp_gpio_set_io(0,0);
        printk("PMU Disenable! \n");
	}
}
static struct mutex wifi_bt_mutex;

static void extern_pmu_set_enable(int is_on, int shift)
{
	DBG_LINE_INFO();
...
{
	DBG_LINE_INFO();
    extern_pmu_set_enable(is_on, WIFI_BIT_SHIFT);
    if(is_on){
        //keep pmu_en high for 30 ms
        gpio_out(PAD_GPIOAO_3,1);
        mdelay(50);
...
{
	DBG_LINE_INFO();
    extern_pmu_set_enable(is_on, BT_BIT_SHIFT);
    if(is_on){
...
MACHINE_START(MESON6_G17_M1801, "Amlogic Meson 6 g17 t102 board")
