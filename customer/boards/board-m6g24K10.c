...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 88,////150,
    .pmu_battery_cap = 7690,///7700,////8000,
    ...
    .pmu_usbcur = 500,
#else
    .pmu_init_chgcur = 800000,		//set initial charging current limite
    .pmu_suspend_chgcur = 1500000,	//set suspend charging current limite
    .pmu_resume_chgcur = 800000,							//set resume charging current limite
...
static u8 gt82x_config[] = {
    0x0F,0x80,/*config address*/
       ...
       0x0A,0x00,0x0B,0x03,0xC0,0x10,0x10,0x20,
       0x00,0x00,0x08,0x00,0x00,0x02,0x50,0x3C,
       0x1C,0x03,0x00,0x05,0x00,GT82X_YRES>>8,GT82X_YRES&0xff,GT82X_XRES>>8,
    GT82X_XRES&0xff,0x55,0x49,0x50,0x44,0x28,0x00,0x05,
       0x19,0x03,0x14,0x10,0x03,0x52,0x01,0x00,
...
static void sdio_extern_init(void)
{
    ...
    gpio_irq_set(PAD_GPIOX_11,GPIO_IRQ(4,GPIO_IRQ_RISING));
		...
		#else //} else {
			gpio_set_status(PAD_GPIOX_16,gpio_status_in); 
			gpio_irq_set(PAD_GPIOX_16,GPIO_IRQ(4,GPIO_IRQ_HIGH));
	//	}
		#endif
	#endif
    #ifdef CONFIG_AM_WIFI
...
{
#ifndef CONFIG_BCM40181_OOB_INTR_ONLY
...
static int modem_power_off(void)
{
	...
	axp_gpio_set_value(0, 1);	//set AXP 202 GPIO0 low ,power off
	gpio_out(PAD_GPIOA_3,1);//GPIOA_3 = power_key
	mdelay(8);
	gpio_out(PAD_GPIOA_2,0); //GPIOA_2 = off_sleep
	mdelay(5);
