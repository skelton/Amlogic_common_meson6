...
static struct axp_cfg_info axp_cfg = {
    ...
    .pmu_battery_rdc = 100,////150,
    .pmu_battery_cap = 7760,////8000,
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G06_1003AC
...
static int bat_int_resistor = 190;
#define MAX_RES_V 250
...
retry:
	...
	//NOT USE current to do calibration for current is very undependable;
...
static int bat_charge_table[57]={
0,  //0    
7295821,//0
7321475,//1
7477875,//2
...
static int bat_discharge_table[57]={
0,      //0
7000000,//0
7008675,//1
7184625,//2
...
static inline int get_bat_percentage(int adc_vaule, int *adc_table, int *per_table, int table_size)
{
	int i;
...
static int low_current_charge_times=0;
#define ALLOW_MAX_DIFF 5 //if percentage change is bigger than this value,do something
#define LOW_CURRENT_CHARGE_FULL 150 //150 times in low current charge ,then report full.
#define LOW_CURRENT 200000 //200mA is low current for charge mode
static int act8942_measure_capacity_charging(void)
{
	int ret=0,cur=0;
	int vbat = _measure_voltage(); 
	int table_size = ARRAY_SIZE(bat_charge_table);
	ret = get_bat_percentage(vbat, bat_charge_table, bat_level_table, table_size);

	/* for some platforms charge can not be full */
	cur = _measure_current();
	if(ret > 90 && (cur < LOW_CURRENT && cur >= 0)){
		low_current_charge_times+=(cur==0)?10:1;
	}
	else{
		low_current_charge_times=0;
	}
	if(low_current_charge_times > LOW_CURRENT_CHARGE_FULL){
		ret = 100;
		printk("Maybe full!!!charge current is %d for low_current_charge_times %d\n",cur,low_current_charge_times);
	}
	//printk("+++charge current=%d   low_current_charge_times=%d\n",cur,low_current_charge_times);
	/* for some platforms charge can not be full */

	//printk("+++charge percentage=%d%%   vbat=%d\n",ret,vbat);
	return ret;
}
...
static int act8942_measure_capacity_battery(void)
{
	int ret=0;
	int vbat = _measure_voltage();
	int table_size = ARRAY_SIZE(bat_discharge_table);

	ret = get_bat_percentage(vbat, bat_discharge_table, bat_level_table, table_size);

	//printk("---discharge percentage=%d%%   vbat=%d\n",ret,vbat);
	return ret;
}
...
static struct act8942_operations act8942_pdata = {
	...
	.update_period = 2500,	//2.5S
	.asn = 20,				//Average Sample Number
...
#if 0
		i2c_register_board_info(1, aml_i2c_bus_info_b_mma8452,
			ARRAY_SIZE(aml_i2c_bus_info_b_mma8452));
#endif
